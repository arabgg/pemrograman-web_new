<!--Praktikum 01 - CRUD Bagian Read -->
<!--Praktikum 05 - Tampilan CRUD dengan Bootstrap -->
<?php
include('koneksi.php'); // Mengimpor file koneksi.php untuk terhubung dengan database
$query = "SELECT * FROM anggota ORDER BY id DESC"; // Query untuk memilih semua data anggota dari tabel anggota dan mengurutkannya berdasarkan id secara descending
$result = mysqli_query($koneksi, $query); // Eksekusi query dan simpan hasilnya dalam variabel $result
?>

<!DOCTYPE html>
<html>
<head>
    <title>Data Anggota</title>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css"> <!-- Mengimpor file CSS Bootstrap -->
</head>
<body>
    <div class="container mt-4">
        <h2>Data Anggota</h2>
        <a class="btn btn-success mt-2" href="create.php">Tambah Data</a> <!-- Tombol untuk menambah data -->
        <br><br>
        <table class="table">
            <thead class="thead-light">
                <tr>
                    <th>No</th>
                    <th>Nama</th>
                    <th>Jenis Kelamin</th>
                    <th>Alamat</th>
                    <th>No. Telp</th>
                    <th>Aksi</th>
                </tr>
            </thead>
            <tbody>
            <?php
            $no = 1; // Inisialisasi variabel $no untuk nomor urut
            while ($row = mysqli_fetch_assoc($result)) { // Looping untuk menampilkan data anggota
                $kelamin = ($row["jenis_kelamin"] == 'L') ? 'Laki-Laki' : 'Perempuan'; // Mengubah jenis kelamin menjadi teks
            ?>
                <tr>
                    <td><?= $no++ ?></td>
                    <td><?= $row["nama"] ?></td>
                    <td><?= $kelamin ?></td>
                    <td><?= $row["alamat"] ?></td>
                    <td><?= $row["no_telp"] ?></td>
                    <td>
                        <a class="btn btn-primary" href="edit.php?id=<?= $row["id"] ?>">Edit</a> <!-- Tombol untuk mengedit data -->
                        <a class="btn btn-danger" href='#' data-toggle='modal' data-target='#hapusModal<?= $row["id"] ?>'>Hapus </a> <!-- Tombol untuk menghapus data -->
                    </td>
                </tr>
                <!-- Modal untuk konfirmasi penghapusan data -->
                <div class='modal fade' id='hapusModal<?= $row["id"] ?>' tabindex='-1' role='dialog' aria-labelledby='exampleMOdalLabel' aria-hidden='true'>
                    <div class='modal-dialog' role='document'>
                        <div class='modal-content'>
                            <div class='modal-header'>
                                <h5 class='modal-title' id='exampleModalLabel'>Konfirmasi Hapus</h5>
                                <button type="button" class="close" data-dismiss='modal' aria-label='Close'> 
                                    <span aria-hidden='true'>&times;</span>
                                </button>
                            </div>
                            <div class='modal-body'> 
                                <?= "Apakah Anda yakin ingin menghapus data dengan nama " . $row["nama"] . "?" ?>
                            </div> 
                            <div class='modal-footer'>
                                <a class='btn btn-danger' href='proses.php?aksi=hapus&id=<?= $row["id"] ?>'>Hapus</a> <!-- Tombol untuk menghapus data -->
                                <button type='button' class='btn btn-secondary' data-dismiss='modal'>Batal</button>
                            </div>
                        </div>
                    </div>
                </div>
            <?php
            }
            ?>
            </tbody>
        </table>
    </div>
    <script src="https://code.jquery.com/jquery-3.5.1.slim.min.js"></script> <!-- Mengimpor jQuery -->
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.5.3/dist/umd/popper.min.js"></script> <!-- Mengimpor Popper.js -->
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script> <!-- Mengimpor file JS Bootstrap -->
</body>
</html>